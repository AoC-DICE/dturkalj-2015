function day9TaskTwo() {
    const INF = Number.MIN_SAFE_INTEGER;
    let maxDistanceFinal = Number.MIN_SAFE_INTEGER;
    ...
    paths.forEach((path, index) => {
        ...
        for (let i = 0; i < path.length - 1; i++) {
            maxDistance += distances[path.charAt(i)][path.charAt(i + 1)];
        }
        if (maxDistance > maxDistanceFinal) maxDistanceFinal = maxDistance;
    });

    console.log(`Maximum cost is ${maxDistanceFinal}`);
